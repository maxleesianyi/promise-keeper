...
export async function sendApprovalCard(promise: { id: string; title: string; dueText: string }) { await callTelegram("sendMessage", { chat_id: process.env.TELEGRAM_USER_ID, text: `I detected a task from The Wife:\n\n*${promise.title}*\n${promise.dueText}\n\nSave it to Do Already?`, parse_mode: "Markdown", reply_markup: { inline_keyboard: [[{ text: "Save promise", callback_data: `save:${promise.id}` }, { text: "Not a promise", callback_data: `ignore:${promise.id}` }]] } }); }
